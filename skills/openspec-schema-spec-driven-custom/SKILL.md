...
**What this schema is**: A TDD-focused variant of the standard `spec-driven` workflow. The apply phase enforces a strict RED → GREEN → REVIEW cycle per task (no refactor step). Identical to `spec-driven` in all other respects (proposal → specs → [design] → tasks).

**Steps**

1. Check whether `openspec/schemas/spec-driven-custom/schema.yaml` already exists in the current project.
   - If it does, confirm with the user before overwriting.

2. Create the directory and write the schema file:

   Path: `openspec/schemas/spec-driven-custom/schema.yaml`
     ...
     - id: tasks
       ...
       instruction: >
         ...
         **IMPORTANT: Follow the template below exactly.** The apply phase parses
         ...
         - Write tasks as observable behaviors, not implementation steps
         ...
         - [ ] 1.1 Login with valid credentials returns a session token

         - [ ] 1.2 Login with invalid credentials returns an error


         ## 2. User can log out


         - [ ] 2.1 Logout invalidates the session token

         ```


         Reference specs for what needs to be built, design for how to build it (if present).
   ...
   apply:
     ...
     tracks: tasks.md
     instruction: |
       Read context files, then implement each task using strict TDD — one behavior at a time.

       For each task:

       1. **RED** — Write a failing test that specifies the behavior. Run the test
          suite and confirm the new test fails. Do not proceed if it passes (the
          behavior already exists or the test is wrong).

       2. **GREEN** — Write the minimal code to make the test pass. Run the test
          suite and confirm it passes without breaking others. No speculative code.

       3. **REVIEW** — Invoke the `code-review` skill, passing the files changed
          in this cycle (not the full diff). Fix every finding inline. Only file a
          GitHub issue when the fix is too large or too risky to do now. Do not
          move to the next task until all findings are fixed or filed.

       4. Mark task complete: `- [ ]` → `- [x]`

       Tests must verify behavior through public interfaces only — not private
       methods or implementation details. A test that breaks on internal rename is
       testing the wrong thing.

       Test fixtures must be deterministic — use fixed constants, not values that
       vary between runs.

       Pause if: the RED test won't fail (resolve with user), GREEN breaks unrelated
